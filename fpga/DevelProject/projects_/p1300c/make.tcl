...
set boards_folder [file normalize [pwd]./]
set ip_folder [file normalize [pwd]/../IP]
set projects_folder [file normalize [pwd]./]
...
if {[string match -nocase "no" $jtag]} {
   ...
   if {[string match -nocase "yes" $sdk]} {
      ...
      cd ${projects_folder}
      exec >@stdout 2>@stderr xsdk -batch -source ../../software_/$project\_sdk.tcl -notrace
      puts "Generating BOOT.BIN..."
      exec >@stdout 2>@stderr bootgen -image ../../software_/$project\_sd.bif -w -o BOOT.bin
      cd ${scripts_folder}
   }
